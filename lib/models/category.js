...
class Category {
    constructor(entity, instance) {
        ...
        this.section = null;
        this.modularDetailsName = null;
        this.modularDetailsContent = null;
    }
    ...
    fromPersistence(Promise, pluginInfo, entity, instance) {
        return Promise.resolve()
            .then(() => {
                ...
                this.modularDetailsName = instance.ModularDetailsName;
                this.modularDetailsContent = instance.ModularDetailsContent;
            })
            ...
            // IconImage
            .then(() => getModelInstances(
                Promise, pluginInfo, entity, instance, pluginInfo.config.schema.iconImage, Imglib
            ))
            .then((imgLibs) => {
                if (imgLibs && imgLibs.length) {
                    const imgLib = imgLibs[0];
                    if (imgLib.images && imgLib.images.length) {
                        const image = imgLib.images[0];
                        this.iconImage = image.id;
                        this.iconImageUrl = image.url;
                    }
                }
            })
            ...
            // logoImage
            .then(() => getModelInstances(
                Promise, pluginInfo, entity, instance, pluginInfo.config.schema.logoImage, Imglib
            ))
            .then((imgLibs) => {
                if (imgLibs && imgLibs.length) {
                    const imgLib = imgLibs[0];
    ...
    toHal(warpjsUtils, RoutesInfo, routes, domain) {
        const resource = warpjsUtils.createResource('', {
            ...
            modularDetailsName: this.modularDetailsName,
            modularDetailsContent: this.modularDetailsContent,
    ...
    static fromHal(json) {
        ...
        instance.modularDetailsName = json.modularDetailsName;
        instance.modularDetailsContent = json.modularDetailsContent;
        instance.iconImage = json.iconImage;
        instance.iconImageUrl = json.iconImageUrl;
        instance.logoImage = json.logoImage;
        instance.logoImageUrl = json.logoImageUrl;
