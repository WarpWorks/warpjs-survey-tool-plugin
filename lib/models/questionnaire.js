...
class Questionnaire {
    ...
    toHal(warpjsUtils, RoutesInfo, routes, domain) {
        ...
        resource.embed('categories', this.categories.map(
            (category) => category.toHal(warpjsUtils)
        ));
        ...
        resource.embed('resultSets', this.resultSets.map(
            (resultSet) => resultSet.toHal(warpjsUtils, RoutesInfo, routes, domain)
        ));
    ...
    generateDefaultAssessment(uuid, assessmentId) {
        ...
        assessment.projectName = "New Project";
    ...
    newResultQuestionFeedback(feedbackId, questionId, resultId, resultsetId, thumbValue, comment) {
