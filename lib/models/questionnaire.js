...
class Questionnaire {
    ...
    toHal() {
        const href = RoutesInfo.expand(constants.routes.questionnaire, { id: this.id });
    ...
    toHalFull(domain, pluginConfig, persistence) {
        return Promise.resolve()
            .then(() => RoutesInfo.expand(constants.routes.newQuestionnaire, {domain: domain, surveyId: this.id}))
            .then((href) => warpjsUtils.createResource(href, {
                id: this.id,
                name: this.name
            }))
            .then((resource) => Promise.resolve()
                .then(() => resource.link('docx', RoutesInfo.expand(constants.routes.docx, {domain: domain, surveyId: this.id})))
                // get categories
                ...
                .then((categoryEntity) => Promise.resolve()
                    ...
                    .then((categoriesHAL) => _.orderBy(categoriesHAL, (halCategory) => {
                        return parseInt(halCategory.position, 10);
                    }))
                )
                ...
                // get results
                .then(() => this.entity.getRelationshipByName(pluginConfig.schema.resultSet))
                .then((resultSetRelationship) => resultSetRelationship ? resultSetRelationship.targetEntity[0] : null)
                .then((resultSetEntity) => Promise.resolve()
                    .then(() => resultSetEntity.getDocuments(persistence))
                    .then((resultSetDocuments) => Promise.resolve()
                        .then(() => resultSetDocuments ? resultSetDocuments.map((resultSetDocument) => new ResultSet(resultSetEntity, resultSetDocument)) : null)
                        .then((resultSetInstances) => resultSetInstances ? Promise.map(resultSetInstances, (resultSetInstance) => resultSetInstance.toHal(domain, pluginConfig, persistence)) : null)
                        .then((resultSetsHAL) => _.orderBy(resultSetsHAL, (halResultSet) => {
                            return parseInt(halResultSet.position, 10);
                        }))
                        .then((resultSetsOrderedHAL) => resultSetsOrderedHAL ? resource.embed('resultSets', resultSetsOrderedHAL) : null)
                    )
                )
                .then(() => resource)
            )
