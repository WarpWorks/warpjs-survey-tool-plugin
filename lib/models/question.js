...
class Question {
    ...
    fromPersistence(Promise, pluginInfo, entity, instance) {
        return Promise.resolve()
            ...
            // QuestionModules
            .then(() => getModelInstances(
                Promise, pluginInfo, entity, instance, 'QuestionModule', QuestionModule
            ))
            .then((questionModules) => {
    ...
    toHal(warpjsUtils, RoutesInfo, routes, domain) {
        ...
        resource.embed('questionModules', this.questionModules.map(
            (questionModule) => questionModule.toHal(warpjsUtils, RoutesInfo, routes)
        ));
    ...
    static fromHal(json) {
        ...
        if (json._embedded) {
            ...
            if (json._embedded.questionModules) {
                instance.questionModules = json._embedded.questionModules.map(QuestionModule.fromHal);
            }
        }
    ...
    generateDefaultAnswer(uuid) {
        ...
        answerQuestion.questionModules = this.questionModules;
