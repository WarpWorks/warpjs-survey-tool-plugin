...
class Question {
    ...
    toHalRelevance(warpjsUtils, RoutesInfo, routes, domain, resultId) {
        const randomNumber = () => {
            return Math.floor(Math.random() * Math.floor(20));
        };

        const thumbsUp = randomNumber();
        const thumbsDown = randomNumber();
        ...
        const resource = warpjsUtils.createResource('', {
            ...
            commentResult: thumbResult()
        });
        resource.link('thumbsup', RoutesInfo.expand(routes.feedbackQuestionDetails, {
            domain,
            resultId: resultId,
            questionId: this.id,
            thumbDirection: 'up'
        }));
        resource.link('thumbsdown', RoutesInfo.expand(routes.feedbackQuestionDetails, {
            domain,
            resultId: resultId,
            questionId: this.id,
            thumbDirection: 'down'
    ...
    toHalResultFeedbackSpecific(warpjsUtils, RoutesInfo, routes, resultId, thumb) {
        ...
        const generateComment = (resource, thumb) => {
            return warpjsUtils.createResource('', {
                thumbDirection: thumb,
                date: date.toString(),
                comment: 'This is a comment. Some more text goes here. It might be long. But maybe not.'
            });
        };

        const feedbackNumber = randomNumber(20);
        const resource = warpjsUtils.createResource('', {
            id: this.id,
            name: this.name
        });
        ...
        for (let i = 0; i < feedbackNumber; i++) {
            comments.push(generateComment(resource, thumb));
        }

        resource.embed('feedback', comments);
